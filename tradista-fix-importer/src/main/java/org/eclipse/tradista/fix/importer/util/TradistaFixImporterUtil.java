package org.eclipse.tradista.fix.importer.util;
...
public final class TradistaFixImporterUtil {
	...
	private TradistaFixImporterUtil() {
	}
	...
	public static Currency parseFixCurrency(FieldMap fieldMap, int tag) {
		...
		try {
			...
			throw new TradistaTechnicalException(
					String.format("There was an issue retrieving the currency in tag %d: %s", tag, tbe.getMessage()));
		}
		return currency;
	}

	...
	public void fixCashTransfers(long quoteSetId) throws TradistaBusinessException {
		...
		if (new QuoteBusinessDelegate().getQuoteSetById(quoteSetId) == null) {
			throw new TradistaBusinessException(
					String.format("The quoteSet %d doesn't exist in the system.", quoteSetId));
		}
		...
		if (transfers != null && !transfers.isEmpty()) {
			for (Transfer transfer : transfers) {
				...
				try {
					TradistaUtil.getTransferManager(productType).fixCashTransfer((CashTransfer) transfer, quoteSetId);

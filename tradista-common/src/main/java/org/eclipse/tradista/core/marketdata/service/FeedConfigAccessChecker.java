...
/**
 * access checker for feed configurations.
 */
public class FeedConfigAccessChecker implements AccessChecker {

	private FeedBusinessDelegate feedBusinessDelegate;

	public FeedConfigAccessChecker() {
	...
	public void check(Object value, StringBuilder errMsg) throws TradistaBusinessException {
		if (value instanceof FeedConfig feedConfig) {
			if (feedConfig.getId() != 0) {
				if (feedBusinessDelegate.getFeedConfigById(feedConfig.getId()) == null) {
					errMsg.append(String.format("The feed config %s was not found.%n", feedConfig.getName()));
				}
			}
		} else if (value instanceof Long id) {
			if (id != 0) {
				if (feedBusinessDelegate.getFeedConfigById(id) == null) {
					errMsg.append(String.format("The feed config with id %d was not found.%n", id));
				}
			}
		}
	}
